<template>
    ...
    <el-dropdown @visible-change="visibleChange" @command="addComment">
        <el-button type="primary" :disabled="!isEdit">标注</el-button>
        <template #dropdown>
            <el-dropdown-menu>
                <el-dropdown-item command="sup">上标注</el-dropdown-item>
                <el-dropdown-item command="sub">下标注</el-dropdown-item>
            </el-dropdown-menu>
        </template>
    </el-dropdown>
...
<script setup>
    ...
    const addComment = (type) => {
        const _tag = type;
        const _tempSelectRange = tempSelectRange;

        ElMessageBox.prompt('请输入评论内容', 'Tip', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
        })
            .then(({ value }) => {
                const selectRange = _tempSelectRange;
                const commentTag = document.createElement(_tag);
                commentTag.setAttribute('comment', value);

                selectRange.surroundContents(commentTag);
            })
            .catch(() => {
